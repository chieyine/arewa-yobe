CREATE OR REPLACE FUNCTION civic.filtered_report_ids(f jsonb) RETURNS SETOF text LANGUAGE plpgsql STABLE SECURITY DEFINER SET search_path=pg_catalog,civic AS $$
...
BEGIN
 ...
 RETURN QUERY SELECT r.id FROM civic.reports r JOIN civic.items i ON i.id=r.item_id JOIN civic.revisions v ON v.id=r.data->>'currentRevisionId'
...
CREATE OR REPLACE FUNCTION civic.report_population(f jsonb) RETURNS TABLE(id text,report jsonb,item jsonb,revision jsonb) LANGUAGE sql STABLE SECURITY DEFINER SET search_path=pg_catalog,civic AS $$
SELECT r.id,r.data,i.data,v.data FROM civic.filtered_report_ids(f) visible(id) JOIN civic.reports r ON r.id=visible.id JOIN civic.items i ON i.id=r.item_id JOIN civic.revisions v ON v.id=r.data->>'currentRevisionId'
$$;
...
BEGIN
 WITH population AS MATERIALIZED (SELECT r.id,r.item_id,i.lga_id,i.data->>'sectorId' sector_id,i.data->>'confirmedProgress' progress,r.data->>'reviewState' state,to_char((r.data->>'firstSubmittedAt')::timestamptz AT TIME ZONE 'Africa/Lagos','YYYY-MM-DD') AS submitted_day FROM civic.filtered_report_ids(f) visible(id) JOIN civic.reports r ON r.id=visible.id JOIN civic.items i ON i.id=r.item_id),
 ...
 trend AS (SELECT submitted_day label,count(*)::int value FROM population GROUP BY 1),
...
REVOKE ALL ON FUNCTION civic.filtered_report_ids(jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION civic.filtered_report_ids(jsonb) TO arewa_app;
INSERT INTO civic.schema_migrations(version) VALUES('006_aggregate_memory');
